...
function useOfferCountdown(storageKey: string) {
  ...
  useEffect(() => {
    const key = `sv-cod-offer:${storageKey}`;
...
/** El Salvador-only Fufills-style COD checkout (not used for other markets). */
export function SalvadorCodCheckout({
  product,
  country,
  qty,
  onQtyChange,
    ...
    >
      ...
      <form ref={formRef} onSubmit={onSubmit} className="space-y-3 p-4 sm:p-5">
              ...
              >
                ...
                <span className="shrink-0 text-end">
                  <span className="block text-base font-extrabold text-[#ff7a00] sm:text-lg">
                    {formatSalvadorCodPrice(offer.totalLocal)}
                  </span>
                  {offer.fullPriceLocal > offer.totalLocal ? (
                    <span className="text-xs text-[#999] line-through">
                      {formatSalvadorCodPrice(offer.fullPriceLocal)}
                    </span>
                  ) : null}
                </span>
              </button>
        ...
        <IconField
          ...
          placeholder="Dirección exacta y con número"
          required
