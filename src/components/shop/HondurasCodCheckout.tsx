...
export function HondurasCodCheckout({
  ...
  const provincias = useMemo(() => hondurasProvincias(), []);
  const municipios = useMemo(
    () => hondurasMunicipios(provincia),
    [provincia]
  );
              ...
              >
                ...
                <span className="shrink-0 text-end">
                  <span className="block text-base font-extrabold text-[#ff7a00] sm:text-lg">
                    {formatHondurasCodPrice(offer.totalLocal)}
                  </span>
                  {offer.fullPriceLocal > offer.totalLocal ? (
                    <span className="text-xs text-[#999] line-through">
                      {formatHondurasCodPrice(offer.fullPriceLocal)}
                    </span>
                  ) : null}
                </span>
              </button>
