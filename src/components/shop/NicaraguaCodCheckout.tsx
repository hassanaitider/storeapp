...
function useOfferCountdown(storageKey: string) {
  ...
  useEffect(() => {
    const key = `ni-cod-offer:${storageKey}`;
...
/** Nicaragua-only Fufills-style COD checkout (not used for other markets). */
export function NicaraguaCodCheckout({
  product,
  country,
  qty,
  onQtyChange,
  ...
  const departamentos = useMemo(() => nicaraguaDepartamentos(), []);
  const municipios = useMemo(
    () => nicaraguaMunicipios(departamento),
    [departamento]
  );
  const barrios = useMemo(
    () => nicaraguaBarrios(departamento, municipio),
  ...
  const totalLabel = formatNicaraguaCodPrice(totalLocal);
    ...
    >
      ...
      <form ref={formRef} onSubmit={onSubmit} className="space-y-3 p-4 sm:p-5">
              ...
              >
                ...
                <span className="shrink-0 text-end">
                  <span className="block text-base font-extrabold text-[#ff7a00] sm:text-lg">
                    {formatNicaraguaCodPrice(offer.totalLocal)}
                  </span>
                  {offer.fullPriceLocal > offer.totalLocal ? (
                    <span className="text-xs text-[#999] line-through">
                      {formatNicaraguaCodPrice(offer.fullPriceLocal)}
                    </span>
                  ) : null}
                </span>
              </button>
        ...
        <IconField
          ...
          placeholder="Ingrese su dirección de envío correcta"
          required
