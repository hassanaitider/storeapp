...
function useOfferCountdown(storageKey: string) {
  ...
  useEffect(() => {
    const key = `ec-cod-offer:${storageKey}`;
...
/** Ecuador-only Fufills-style COD checkout (not used for other markets). */
export function EcuadorCodCheckout({
  product,
  country,
  qty,
  onQtyChange,
              ...
              >
                ...
                <span className="shrink-0 text-end">
                  <span className="block text-base font-extrabold text-[#ff7a00] sm:text-lg">
                    {formatEcuadorCodPrice(offer.totalLocal)}
                  </span>
                  {offer.fullPriceLocal > offer.totalLocal ? (
                    <span className="text-xs text-[#999] line-through">
                      {formatEcuadorCodPrice(offer.fullPriceLocal)}
                    </span>
                  ) : null}
                </span>
              </button>
