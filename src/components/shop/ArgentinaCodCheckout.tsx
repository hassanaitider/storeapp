...
function useOfferCountdown(storageKey: string) {
  ...
  useEffect(() => {
    const key = `ar-cod-offer:${storageKey}`;
...
/** Argentina-only Fufills-style COD checkout (not used for other markets). */
export function ArgentinaCodCheckout({
  product,
  country,
  qty,
  onQtyChange,
  ...
  const provincias = useMemo(() => argentinaProvincias(), []);
    ...
    >
      ...
      <div className="bg-[#121c2d] px-4 pb-5 pt-5 sm:px-5">
        ...
        <p className="mt-1.5 text-xs font-medium tracking-wide text-white/55 sm:text-[13px]">
          AR COD FORM ARGENTINA · Tu producto
        </p>
      </div>
      ...
      <form ref={formRef} onSubmit={onSubmit} className="space-y-3 p-4 sm:p-5">
        ...
        {upsellOn ? (
          <LatamCodQtyPacks
            ...
            formatPrice={formatArgentinaCodPrice}
          />
        ) : null}
...
function SelectField({
  ...
  return (
    ...
    >
      <MapPin className="pointer-events-none absolute start-3 h-4 w-4 text-[#9a9a9a]" />
      <select
        required={required}
        disabled={disabled}
        value={value}
